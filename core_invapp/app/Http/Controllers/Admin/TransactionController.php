    ...
    private function updateStatus($transaction, $request)
    {
        ...
        if (!empty($status)) {
            $this->wrapInTransaction(function ($transaction, $request, $status) {
                ...
                $transaction->fresh();
                $etype = ($transaction->type == TransactionType::DEPOSIT) ? 'deposit' : (($transaction->type == TransactionType::WITHDRAW) ? 'withdraw' : false);
                ...
                if ($status == TransactionStatus::CONFIRMED && $transaction->type == TransactionType::WITHDRAW) {
                    ...
                    try {
                        if ($etype) {
                            ProcessEmail::dispatch($etype.'-confirmed-customer', data_get($transaction, 'customer'), null, $transaction);
                    ...
                    try {
                        if ($etype) {
                            ...
                            ProcessEmail::dispatch($etype. $state .'-customer', data_get($transaction, 'customer'), null, $transaction);
